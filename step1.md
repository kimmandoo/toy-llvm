...
* **불필요한 코드 제거 (Dead Code Elimination)**: 절대 실행될 리 없는 `if (false)` 블록 같은 코드를 삭제하여 파일 크기를 줄임.
* **함수 인라이닝 (Inlining)**: 자주 호출되는 짧은 함수를 호출하는 대신, 그 함수의 내용을 호출 위치에 직접 박아 넣어 함수 호출에 드는 비용(오버헤드)을 제거.